你要为“小动物夜谈会”融合并维护用户的长期状态画像。

这是两阶段长期画像更新的第二阶段。第一阶段已经提取本次 session 的六领域观察；你现在要把这些观察与旧画像、历史版本和长期记忆融合，形成当前最完整而不过度推断的理解。
...
可用于补足跨 session 背景的长期记忆：
{long_term_memories}

第一阶段提取的本次 session 观察：
{session_observations}

请逐一审阅全部 domain，并判断当前完整画像是否需要创建或更新。
...
- 即使本次 session 很短、没有新信息，也必须输出六个 domain，并使用 no_change。
...
- 某个 domain 还没有旧画像时，可以根据高置信度长期记忆与本次观察共同 create；不能仅凭领域名称猜测。
- 不要诊断用户；用“模式、阶段、倾向、需要、策略”来描述。
- 不要把一次性情绪当作长期状态，除非它连接到反复模式或明显阶段变化。
- 判断 update 时要同时参考当前画像、最近历史版本、长期记忆和本次观察；不要只因为本次 session 出现强烈情绪就覆盖长期状态。
- update 的 summary 必须是整合旧画像与本次新证据后的“当前完整理解”，不是只描述本次 session 的增量摘要。仍然成立的旧线索必须保留；已经被新证据修正的旧判断可以明确改写。
- evidence 只列第一阶段提取的本次 session 新增证据；系统会把它与历史证据去重合并。
