...
def call_llm_for_mental_status(
    llm: Any,
    text: str,
    filename: str = "",
) -> dict[str, Any] | None:
    prompt = _read_prompt("mental_status_extract.md").format(
        moods="、".join(MENTAL_STATUS_MOODS),
    )
    context = text if len(text) <= 4000 else text[:4000]
    if filename:
    ...
    try:
        ...
        status = _parse_json_object(response.content)
        if not status.get("mood"):
            LOGGER.warning("LLM returned empty mood for %s", filename)
            return None
        return status
    except Exception:
...
class DiaryImporter:
    ...
    def _upsert_source(self, file_path: Path, file_hash: str, record_id: str, record_date: str) -> None:
            ...
            if existing:
                ...
                conn.execute(
                    ...
                    (id, source_type, file_path, file_hash, extracted_record_ids,
                     last_imported_at, document_date)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
    ...
    def import_diaries(
        ...
        for idx, file_path in enumerate(files, 1):
            try:
                ...
                status = call_llm_for_mental_status(
                ...
                if not dry_run:
                    record_id = self.store.add_mental_status_record(record)
                    self._upsert_source(file_path, file_hash, record_id, record_date)
                else:
                    preview = {k: v for k, v in record.items() if v not in (None, "", {})}
                    LOGGER.info("  [dry-run] LLM result:\n%s", json.dumps(preview, ensure_ascii=False, indent=4))
        ...
        LOGGER.info(
            "diary import done: imported=%d skipped=%d errors=%d total=%d",
            imported_count,
            skipped_count,
            error_count,
            len(files),
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Import personal diary into mental status records via LLM")
