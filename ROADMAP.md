...
- 长期记忆页必须覆盖数据库中现有的全部类别；每条记忆的核心字段应可访问。
...
1. **M0 基线与盘点**：Mac 构建可运行；形成关键路径性能基线和数据字段矩阵。
2. **M1 稳定性止血**：处理可复现卡死、慢点击、主线程数据库和重复全量计算。
3. **M2 双向互融闭环**：完成心流 ↔ 夜谈的相关内容展示、点击进入、更新与空状态。
4. **M3 数据展示闭环**：长期记忆类别/字段和会后总结关联日记完整可访问。
5. **M4 体验验收**：用真实但脱敏的数据场景完成 Mac 人工验收和性能复测。

以下内容保留为项目早期判断和长期背景；若与上面的当前主线冲突，以上述 Mac M0-M4 为准。
